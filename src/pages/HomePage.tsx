...
import { Activity, Database, ShieldCheck, Zap, AlertCircle, History, Cpu } from 'lucide-react';
import type { SystemMetrics, LogEvent } from '@shared/types';
...
export function HomePage() {
  ...
  const checkpointMutation = useMutation({
    mutationFn: (v: number) => api('/api/checkpoints', { 
      method: 'POST', 
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground flex items-center gap-2">
            ...
            Edge Node Operational ��� ALPHA-1
          </p>
        </div>
        <div className="flex items-center gap-2">
           <Badge variant="outline" className="glass py-1 px-3 border-emerald-500/30 text-emerald-500">
             HEALTHY
           </Badge>
           <Button variant="outline" size="sm" className="glass" onClick={() => queryClient.invalidateQueries()}>
             Refresh Pulse
           </Button>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Throughput", val: "1.2k/min", icon: Activity, color: "text-purple-500" },
          { label: "Cache Hit", val: `${((metrics?.cacheHitRate || 0) * 100).toFixed(0)}%`, icon: Cpu, color: "text-orange-500" },
          { label: "Reliability", val: `${metrics?.uptime}%`, icon: ShieldCheck, color: "text-emerald-500" },
        ].map((item, idx) => (
          ...
          <CardContent className="h-[350px] pl-2">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                />
                <Line type="stepAfter" dataKey="ms" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={{ r: 4, fill: "hsl(var(--primary))", strokeWidth: 0 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <Card className="glass border-border/50">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <p className="text-2xs font-semibold uppercase text-muted-foreground">Versioned Checkpoint</p>
                <div className="flex gap-2">
                  ...
                  />
                  <Button size="sm" className="h-8 gap-2" onClick={() => checkpointMutation.mutate(Number(cpVersion))}>
            ...
            <CardContent>
              <div className="space-y-4 max-h-[220px] overflow-auto pr-2 custom-scrollbar">
                {metrics?.recentLogs.map((log) => (
                  <div key={log.id} className="flex gap-3 items-start border-l-2 border-border/30 pl-3 py-1">
                    <div className="flex-1 space-y-0.5">
                      <p className="text-xs font-medium leading-tight">{log.message}</p>
                      <div className="flex items-center gap-2">
                        <span className={cn(
                          "text-[9px] uppercase font-bold tracking-tighter",
                          log.type === 'ingest' ? "text-blue-400" : "text-emerald-400"
                        )}>
                          {log.type}
                        </span>
                ...
                {!metrics?.recentLogs.length && (
                  <p className="text-xs text-center py-10 text-muted-foreground">Listening for events...</p>
                )}
              </div>
            </CardContent>
          </Card>
